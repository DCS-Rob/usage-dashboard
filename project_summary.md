...
|---------|-------------|-----------|
| 0.6.1   | 2026-05-25  | Netlify auto-deploy gestopt via `netlify.toml` (ignore = "exit 1"). Netlify-statuspaneel toegevoegd in Settings: toont sync-status (lokaal vs Netlify), deploy-knop met credits-waarschuwing + build hook URL invoer. agents-controller auto-sync via systemd timer (elke 5 min git pull). Cache → v16. |
